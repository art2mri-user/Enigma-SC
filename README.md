...
After clicking on the **Spinal Cord Segmentation** button, select the container platform you have installed (Docker or Singularity/Apptainer). Follow the steps on the image below to select all the subfolders inside of the **PREPARED FOLDER** corresponding to the previously selected .nii.gz images or BIDS dataset, instead of selecting files from the input folder. All the files generated by this step, will be placed inside of each individual folder inside of the **PREPARED FOLDER** folder.  
...
You can check an anatomical reference image to compare your results in the section [Anatomical Ground Truth](/Anatomical%20Ground%20Truth.md).  
